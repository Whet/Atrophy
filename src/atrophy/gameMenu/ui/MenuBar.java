...
import atrophy.gameMenu.ui.popups.MainMenuPopup;
...
public class MenuBar extends Crowd{
	...
	private void addTextButtons(final WindowManager windowManager, final MapManager mapWar, final Missions missions, final Squad squad, final ShopManager shopManager, final StashManager stashManager, final TechTree techTree, final ItemMarket itemMarket) {
		...
		TextButton saveGame = new TextButton(TextButton.DEFAULT_ON_COLOUR,TextButton.DEFAULT_OFF_COLOUR) {
			...
			public boolean mD(Point mousePosition, MouseEvent e) {
				...
				if(returnValue == JFileChooser.APPROVE_OPTION){
					SaveFile.saveGame(chooser.getSelectedFile(), squad, missions, mapWar.getSectors(), stashManager.getItems(), missions.getMemCodes(), windowManager);
		...
		TextButton mainMenuButton = new TextButton(TextButton.DEFAULT_ON_COLOUR,TextButton.DEFAULT_OFF_COLOUR) {
			...
			@Override
			public boolean mD(Point mousePosition, MouseEvent e) {
				windowManager.addPopup(null, new MainMenuPopup(windowManager));
				return true;
			}
		};
		mainMenuButton.setLocation(this.getLocation()[0] + 4, this.getLocation()[1] + 115);
		this.addMouseActionItem(mainMenuButton);
		this.addDisplayItem(mainMenuButton);
		...
		exitGame.setLocation(this.getLocation()[0] + 4, this.getLocation()[1] + 135);
