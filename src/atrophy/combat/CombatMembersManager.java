...
import atrophy.combat.ai.DaemonSpider;
import atrophy.combat.ai.DaemonSpiderCommander;
import atrophy.combat.ai.DaemonSpiderling;
	...
	public void addAi(Ai ai) {
		switch(ai.getFaction()) {
			...
			case DAEMON:
				...
				DaemonCommander daemonCommander = null;
				
				// Special case for spider boss, needs a spider commander
				if(ai instanceof DaemonSpider || ai instanceof DaemonSpiderling) {
					if(this.getCommander(Faction.DAEMON) == null) {
						daemonCommander = new DaemonSpiderCommander(aiCrowd.getSquad(), turnProcess, levelManager);
						commanders.add(daemonCommander);
					}
					else {
						daemonCommander = (DaemonCommander) this.getCommander(Faction.DAEMON);
					}
				}
				else
					daemonCommander = this.createDaemonCommander();
